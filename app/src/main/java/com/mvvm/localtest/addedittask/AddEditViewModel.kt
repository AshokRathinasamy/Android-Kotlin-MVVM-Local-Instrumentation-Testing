...
class AddEditViewModel(application: Application) : AndroidViewModel(application) {
    private val tasksRepository = (application as TaskApplication).taskRepository
    ...
    fun saveTask() {
        ...
        if (Task(title = currentTitle, description = currentDescription).isEmpty) {
            _snackbarText.value = Event(R.string.empty_task_message)
            return
        }
        ...
        if (isNewTask || currentTaskId == null) {
            createTask(Task(title = currentTitle, description = currentDescription))
        } else {
            val task = Task(title = currentTitle, description = currentDescription, complete = taskCompleted, id = currentTaskId)
            updateTask(task)
        }
    }
